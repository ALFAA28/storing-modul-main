...
export default function UploadModal({ isOpen, onClose, onUploadSuccess }) {
    ...
    if (isOpen) {
      const fetchMapels = async () => {
        try {
          ...
          console.warn('Gagal memuat mapel dari backend, fallback ke mock mapel:', err);
          setMapels([
            { id: 1, nama_mapel: 'Sejarah Indonesia', tingkat_kelas: '10' },
            { id: 2, nama_mapel: 'Pendidikan Agama Islam', tingkat_kelas: '11' }
          ]);
        }
      };
      fetchMapels();
  ...
  return (
    <div className="fixed inset-0 z-50 overflow-y-auto flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm transition-opacity duration-300">
      <div className="bg-white rounded-2xl shadow-premium border border-slate-200 w-full max-w-lg overflow-hidden animate-all-custom">
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-4">
          ...
          {success && (
            <div className="bg-emerald-50 border border-emerald-250 text-emerald-800 p-4 rounded-xl flex items-center gap-3 animate-pulse">
              <CheckCircle className="w-5 h-5 text-emerald-500 shrink-0" />
              <div className="text-sm font-semibold">Dokumen berhasil diunggah! Memperbarui data...</div>
            </div>
          )}
          ...
          <div>
            ...
            >
              ...
              {file ? (
                ...
                <div className="space-y-2">
                  <div className="bg-indigo-50 text-indigo-550 p-3 rounded-full inline-block">
          ...
          <div className="flex items-center justify-end gap-3 pt-4 border-t border-slate-100">
            <button
              ...
              onClick={onClose}
              disabled={loading || success}
              className="px-4 py-2.5 rounded-xl text-xs font-bold text-slate-650 hover:bg-slate-100 transition-colors border border-slate-200 cursor-pointer"
            >
              Batalkan
            </button>
            <button
              type="submit"
              disabled={loading || success}
              className="px-5 py-2.5 rounded-xl text-xs font-bold bg-indigo-650 hover:bg-indigo-750 text-white shadow-md shadow-indigo-600/10 hover:shadow-indigo-600/20 transition-all flex items-center gap-2 cursor-pointer disabled:opacity-50"
            >
              {loading ? (
                <>
